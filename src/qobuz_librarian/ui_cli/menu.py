...
def interactive_session_mode():
    ...
    log.info(fmt(C.WHITE, "    5) Repair       — re-download damaged (truncated) tracks you own"))
    log.info(fmt(C.WHITE, "    6) Upgrade      — better-quality versions of albums you own"))
    log.info(fmt(C.WHITE, "    7) Migrate      — one-time setup: reorganise an existing library into the"))
    log.info(fmt(C.GRAY,  "                      Artist/Album layout (copies by default, never touches originals)"))
    log.info(fmt(C.WHITE, "    8) Downsample   — bring hi-res files you own down to CD quality to reclaim space"))
