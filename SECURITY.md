...
- **Internet exposure.** The built-in login is a single shared credential. It throttles repeated failures (five per hour per IP, then a `429`), which is appropriate for a trusted home LAN. If you expose the UI to the internet, put it behind a reverse proxy, VPN, or Tailscale. A minimal Caddy example:
